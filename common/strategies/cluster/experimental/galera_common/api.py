...
class GaleraCommonCluster(cluster_models.Cluster):
    ...
    def _create_instances(context, db_info, datastore, datastore_version,
        ...
        LOG.debug("Initiating creation of instance")
    ...
    def _get_cluster_network_interfaces(self):
        ...
        LOG.debug("locha2, %s", nova_instance_id)
        servers = nova_client.servers
        LOG.debug("servers %s",vars(servers))
        interfaces = nova_client.interface.list(nova_instance_id)
        LOG.debug("gol-gappa %s." % interfaces)
